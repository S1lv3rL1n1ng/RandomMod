...
public class VoidPlaneSkyProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, ResourceKey<Level> dimension) {
		if (dimension == null)
			return;
		if (dimension == (ResourceKey.create(Registries.DIMENSION, new ResourceLocation("new_ridiculous_mod_for_the_hahas:void_plane")))) {
			RenderSystem.enableBlend();
			RenderSystem.defaultBlendFunc();
			{
				ResourceLocation _texturelocation = new ResourceLocation(("new_ridiculous_mod_for_the_hahas" + ":textures/" + "night" + ".png"));
				RenderSystem.setShaderTexture(0, _texturelocation);
				Minecraft.getInstance().getTextureManager().bindForSetup(_texturelocation);
			}
			renderSkybox(2, 0, 0, 0, (int) (255 << 24 | 255 << 16 | 255 << 8 | 255), true);
			RenderSystem.enableBlend();
			RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
			renderStars(1500, 10842, 90, (float) (world.getTimeOfDay(Minecraft.getInstance().getPartialTick()) * 360.0F), 0, (int) (255 << 24 | 255 << 16 | 255 << 8 | 255), true);
			{
				ResourceLocation _texturelocation = new ResourceLocation(("new_ridiculous_mod_for_the_hahas" + ":textures/" + "moon_phases" + ".png"));
				RenderSystem.setShaderTexture(0, _texturelocation);
				Minecraft.getInstance().getTextureManager().bindForSetup(_texturelocation);
			}
			RenderSystem.enableBlend();
			RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
			renderMoon(40, (float) (world.getTimeOfDay(Minecraft.getInstance().getPartialTick()) * 360.0F), (int) (255 << 24 | 255 << 16 | 255 << 8 | 255), true, true);
			{
				ResourceLocation _texturelocation = new ResourceLocation(("new_ridiculous_mod_for_the_hahas" + ":textures/" + "constelations" + ".png"));
				RenderSystem.setShaderTexture(0, _texturelocation);
				Minecraft.getInstance().getTextureManager().bindForSetup(_texturelocation);
			}
			RenderSystem.enableBlend();
			RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
			renderTexture(960, 0, 270, 0, (int) (255 << 24 | 255 << 16 | 255 << 8 | 255), true);
		}
	}
}
